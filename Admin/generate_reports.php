<?php
	...
	{  
		...
		$sql_query="SELECT * FROM request ORDER BY request_ID ASC";  
	...
	{  
		$output_date= '';
		$connect = mysqli_connect("localhost", "root", "", "ictc_obar");    
		$sql_query="SELECT * FROM request ORDER BY date_from AND ORDER BY date_to";  
		$result_query=mysqli_query($connect,$sql_query);
		...
		<div>
			<table class="status_table" border=1>
				...
				<?php
					if(isset($_POST['print_by_status']))
					{
						echo fetch_data_status();
					}
					elseif(isset($_POST['print_by_date']))
					{
						echo fetch_data_date();
					}
					else
					{
						echo fetch_data();
